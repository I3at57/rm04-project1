...
On peut vérifier l'hypothèse de la loi exponentielle par un
[Test d'ajustement du $\chi^2$](https://www.bibmath.net/dico/index.php?action=affiche&quoi=c/chideuxtest.html).

Je vais tester effectuer le test suivant :

$$
\left\{
  \begin{array}{ll}
    H_0 : \mbox{les données sont issues d'une loi exponentielle de paramètre }
    \lambda_0 = \hat{\lambda}_{estim} = \frac{1}{\hat{m}}\\
    H_1 : \mbox{les données ne sont pas issues d'une loi exponentielle de paramètre }
    \lambda_0 = \hat{\lambda}_{estim} = \frac{1}{\hat{m}}
  \end{array}
\right.
$$

en prenant $\alpha = 0.95$